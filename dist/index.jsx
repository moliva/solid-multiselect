import { createEffect, createSignal, mergeProps, splitProps, onMount, Show, For } from 'solid-js';
...
export function MultiSelect(props) {
    ...
    function renderGroupByOptions() {
        ...
        return (<For each={groupedObjectKeys}>
                {(objKey) => <>
                        <li class="groupHeading" style={style['groupHeading']}>{objKey}</li>
                        <For each={groupedObject()[objKey]}>
                            {(option) => <li style={style['option']} class="groupChildEle option" classList={{ disableSelection: fadeOutSelection(option) || isDisablePreSelectedValues(option) }} onClick={onSelectItem(option)}>
                                    <Show when={showCheckbox && !singleSelect}>
                                        <input type="checkbox" class="checkbox" readOnly checked={isSelectedValue(option)}/>
                                    </Show>
                                    {isObject ? (<Dynamic component={() => renderValue(option)}/>) : (option || '').toString()}
                                </li>}
                        </For>
                    </>}
            </For>);
    ...
    const isSelectedValue = (item) => {
        if (props.isObject) {
            return (selectedValues().filter((i) => i[props.displayValue] === item[props.displayValue])
                .length > 0);
        }
    ...
    const isDisablePreSelectedValues = (value) => {
        ...
        if (props.isObject) {
            return (preSelectedValues().filter(i => i[props.displayValue] === value[props.displayValue])
                .length > 0);
        }
    ...
    const removeSelectedValuesFromOptions = (skipCheck) => {
        ...
        if (props.isObject) {
            const optionList = unfilteredOptions().filter(item => {
                return selectedValues().findIndex(v => v[props.displayValue] === item[props.displayValue]) === -1
                    ? true
                    : false;
            });
            if (props.groupBy) {
    ...
    function renderNormalOption() {
        return (<For each={options()} fallback={<span style={props.style['notFound']} class="notFound">
                    {props.emptyRecordMsg ?? 'No Options Available'}
                </span>}>
                {(option, index) => <li style={props.style['option']} class="option" classList={{
    ...
    function renderOptionList() {
        ...
        if (props.loading) {
            return (<ul class="optionContainer" style={props.style['optionContainer']}>
    ...
    function renderSelectedList() {
        return (<For each={selectedValues()}>
                {(value) => <span class="chip" classList={{
                    ...
                    disableSelection: isDisablePreSelectedValues(value)
                        ...
                        <Show when={!isDisablePreSelectedValues(value)}>
                            <Show when={!props.customCloseIcon} fallback={<i class="custom-close" onClick={() => onRemoveSelectedItem(value)}>{props.customCloseIcon}</i>}>
    ...
    function renderMultiSelectContainer() {
        return (<div class="multiSelect-container multiSelectContainer" classList={{ disable_ms: disable }} id={id || 'multiSelectContainerSolid'} style={style['multiSelectContainer']}>
                <div class="search-wrapper searchWrapper" classList={{ singleSelect }} ref={searchWrapper} style={style['searchBox']}>
                    {renderSelectedList()}
                    <input type="text" ref={searchBox} class="searchBox" classList={{ searchSingle: singleSelect }} id={`${id || 'search'}_input`} onInput={onInput} value={inputValue()} onFocus={onFocus} onBlur={onBlur} placeholder={((singleSelect && selectedValues().length) || (hidePlaceholder && selectedValues().length)) ? '' : placeholder} onKeyDown={onArrowKeyNavigation} style={style['inputField']} autocomplete="off" disabled={disable}/>
                    <Show when={(singleSelect || showArrow)}>
                        <img class="icon_cancel icon_down_dir" src={DownArrow}/>
                    </Show>
                </div>
                <div class="optionListContainer" classList={{ displayBlock: toggleOptionsList(), displayNone: !toggleOptionsList() }}>
                    {renderOptionList()}
                </div>
            </div>);
    }
